...
def pruning_morphological(trained_model, x_train, y_train, x_val, y_val, x_test, y_test, p, batch_size, extreme=False, optimizer=None, metrics=None, loss=None, finetuning=False):
    """
    ...
    for morph_layer, layer_config in zip(morph_layers, morph_layers_config):
        ...
        #--------------------------------
        # COMPUTE EXTREME PRUNING MASC V2
        #--------------------------------
        _p_masc_extra = activation_count.copy()/activation_count.max()
        _p_masc_extra_M = _p_masc_extra > (p/100)
        _p_masc_extra = _p_masc_extra_M.astype(dtype)
    ...
    #--------------------------
    if extreme:
        print('\nExtreme pruned model with a TEP threshold of ' + str(int(p)) +'%'+' . Test acuraccy')
